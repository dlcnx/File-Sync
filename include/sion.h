...
public:
  ...
  String(char arg) { this->push_back(arg); }
  ...
  std::vector<String> Split(String flag, int num = -1, bool skip_empty = true) const {
    ...
    // find all positions of all flags
    auto pos = FindAll(flag, num);
    if (pos.size() == 0) {
      // if not have flags in this string
      return {*this};
    }
    ...
    for (auto i = 0; i < pos.size() + 1; i++) {
      ...
      if (data_set.size() == num && pos.size() > num && num != -1) {
        push_data(this->substr(pos[data_set.size()] + flag.size()));
        break;
      }
      if (i == 0) { 
        // corner case: 从index 0 到第一个flag
        // 第一个数的位置不是0的话补上
        push_data(this->substr(0, pos[i]));
      } else if (i != pos.size()) {
        int left = pos[i - 1] + flag.size();
        int count = pos[i] - left;
        push_data(this->substr(left, count));
      } else { 
        // 最后一个标志到结束
        push_data(this->substr(*(--pos.end()) + flag.size()));
      }
    }
    return data_set;
  }
